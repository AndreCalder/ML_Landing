...
function AboutUsCard({ img, name, role, description, academicInfo }: { img: string, name: string, role: string, description: string, academicInfo: any }) {

    const [isFlipped, setIsFlipped] = React.useState<boolean>(false);

    const handleFlip = () => setIsFlipped(!isFlipped);
    return (
        <div className={`min-w-64 sm:min-w-96 w-fit flip-card h-full ${isFlipped && "flipped"}`}>
            <div className="flip-card-inner w-full h-full flex items-center">
                <div className="flip-card-front flex flex-col justify-center items-center rounded-[20px] bg-white pt-10 px-5 shadow-md border-b-4 border-ml_orange">
                    <Image src={img} width={250} height={250} alt="Fernando Suárez" />
                    <br />
                    ...
                    <p className="text-ml_orange text-sm flex items-center justify-center my-4 cursor-pointer" onClick={() => handleFlip()}>
